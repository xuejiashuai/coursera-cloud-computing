...
aws autoscaling create-auto-scaling-group --auto-scaling-group-name ${13} --launch-template LaunchTemplateId=${LAUNCHTEMPLATEID} --min-size ${14} --max-size ${15} --desired-capacity ${16} --vpc-zone-identifier "${SUBNET2A},${SUBNET2B}" --tags Key=module,Value=${17} Key=Name,Value=${13} --target-group-arns ${TARGETARN} --health-check-type EC2
