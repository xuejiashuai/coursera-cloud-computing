...
if [ $# = 0 ]
    ...
    RDS_Address=$(aws rds describe-db-instances --db-instance-identifier ${22} --query "")
    echo $RDS_Address
    echo "Retrieving the RDS Read Replica Endpoint Address and printing to the screen..."
    RDS_RR_Address=$(aws rds describe-db-instances --db-instance-identifier ${22}-read-replica --query "")
    echo $RDS_RR_Address
# End of main if
fi
